...
func TestReadOnlyFilterAdditionalAllowed(t *testing.T) {
	filter := NewReadOnlyFilter([]string{"CustomTool"})
...
func TestFullAccessFilter(t *testing.T) {
	filter := NewFullAccessFilter(nil)
...
func TestCustomFilterModes(t *testing.T) {
	whitelist, err := NewCustomFilterWithMode([]string{"Read", "LS"}, nil, "whitelist")
	if err != nil {
	...
	blacklist, err := NewCustomFilterWithMode(nil, []string{"Bash"}, "blacklist")
	if err != nil {
		t.Fatalf("NewCustomFilterWithMode() error = %v", err)
	}
...
func TestCustomFilterInvalidMode(t *testing.T) {
	_, err := NewCustomFilterWithMode(nil, nil, "invalid")
	if err == nil {
