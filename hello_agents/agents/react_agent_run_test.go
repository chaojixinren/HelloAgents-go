...
func TestReActAgentRunBasicFlow(t *testing.T) {
	...
	agent, err := NewReActAgent("react-test", llm, "", cfg, registry, 10)
	if err != nil {
...
func TestReActAgentRunWithMaxStepsExceeded(t *testing.T) {
	...
	agent, err := NewReActAgent("react-test", llm, "", cfg, registry, 2)
	if err != nil {
		t.Fatalf("NewReActAgent error: %v", err)
	}
...
func TestReActAgentArun(t *testing.T) {
	...
	cfg := noTraceConfig()
	agent, err := NewReActAgent("react-test", llm, "", cfg, nil, 5)
	if err != nil {
		t.Fatalf("NewReActAgent error: %v", err)
	}
