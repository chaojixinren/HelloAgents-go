...
func TestBaseAgentArunUsesRunDelegate(t *testing.T) {
	...
	agent, err := NewBaseAgent("tester", llm, "", &cfg, nil)
	if err != nil {
	...
	result, err := agent.Arun("hello", Hooks{}, nil)
	if err != nil {
...
func TestBaseAgentConvertParameterTypesMatchesPythonSemantics(t *testing.T) {
	...
	agent, err := NewBaseAgent("tester", llm, "", &cfg, registry)
	if err != nil {
		t.Fatalf("NewBaseAgent() error = %v", err)
	}

	converted := agent.convertParameterTypes("typed_tool", map[string]any{
		"num":   "12.3abc",
		"count": "7.9",
		"flag":  []any{},
	})
	...
	convertedTruthy := agent.convertParameterTypes("typed_tool", map[string]any{
	...
	convertedStringBool := agent.convertParameterTypes("typed_tool", map[string]any{
		"flag": " true ",
	})
	...
	if got := agent.mapParameterType(" number "); got != "string" {
...
func TestBaseAgentCreateLightLLMUsesEnvLikePython(t *testing.T) {
	...
	llm := &HelloAgentsLLM{
		...
		Provider:    "mock",
		APIKey:      "main-key",
		BaseURL:     "https://main-base.example/v1",
		Temperature: 0.2,
		Timeout:     30,
	}
...
func TestBaseAgentGetSummaryLLMUsesSummaryConfigAndEnv(t *testing.T) {
	...
	agent, err := NewBaseAgent("tester", mainLLM, "", &cfg, nil)
	if err != nil {
		t.Fatalf("NewBaseAgent() error = %v", err)
	}

	summaryLLM, err := agent.getSummaryLLM()
	if err != nil {
...
func TestExtractToolsFromHistoryKeepsEmptyFunctionNameLikePythonSet(t *testing.T) {
	...
	toolsUsed := agent.extractToolsFromHistory(history)
...
func TestAddMessagePanicsWhenAutoSaveIntervalIsZeroLikePythonModuloError(t *testing.T) {
	...
	agent.AddMessage(NewMessage("hello", MessageRoleUser, nil))
}
...
func TestRunAsSubagentKeepsMainHistoryIsolated(t *testing.T) {
	llm := &HelloAgentsLLM{Model: "mock-model", Provider: "mock"}
	cfg := DefaultConfig()
	cfg.TraceEnabled = false
	cfg.SessionEnabled = false
	cfg.SkillsEnabled = false
	...
	agent.AddMessage(NewMessage("main-user", MessageRoleUser, nil))
	agent.AddMessage(NewMessage("main-assistant", MessageRoleAssistant, nil))
	original := agent.GetHistory()

	agent.SetRunDelegate(func(inputText string, kwargs map[string]any) (string, error) {
		agent.AddMessage(NewMessage(inputText, MessageRoleUser, nil))
		agent.AddMessage(NewMessage("subagent-result", MessageRoleAssistant, nil))
