...
// MockLLMAdapter is a test adapter that returns canned responses
...
func (m *MockLLMAdapter) Invoke(messages []map[string]any, kwargs map[string]any) (LLMResponse, error) {
	m.LastMessages = messages
	m.InvokedCount++

	if m.FailOnInvoke || (m.FailOnNthCall > 0 && m.InvokedCount == m.FailOnNthCall) {
		return LLMResponse{}, fmt.Errorf("mock LLM error")
	}
...
func (m *MockLLMAdapter) LastStats() *StreamStats {
	return nil
}

// NewMockLLM creates a HelloAgentsLLM with a mock adapter for testing.
func NewMockLLM(responses ...string) *HelloAgentsLLM {
	adapter := NewMockLLMAdapter(responses...)
...
// NewMockLLMWithToolCalls creates a mock LLM that returns tool_calls in its response.
func NewMockLLMWithToolCalls(toolCallResponses []map[string]any, finalResponse string) *HelloAgentsLLM {
	adapter := &MockLLMAdapter{
		ToolResponses: toolCallResponses,
...
// mockToolCallResponse creates a tool call response payload for mock tests.
...
// mockTextResponse creates a simple text response payload for mock tests.
