...
func TestWriteEditMultiEditResolvePathNormalizesBackslashes(t *testing.T) {
	...
	writeTool := builtin.NewWriteToolWithOptions(root, root, nil)
	editTool := builtin.NewEditToolWithOptions(root, root, nil)
	multiEditTool := builtin.NewMultiEditToolWithOptions(root, root, nil)

	want := filepath.Clean(filepath.Join(root, "nested", "file.txt"))

	if got := filepath.Clean(writeTool.ExportResolvePath(`nested\file.txt`)); got != want {
		t.Fatalf("WriteTool.resolvePath() = %q, want %q", got, want)
	}
	if got := filepath.Clean(editTool.ExportResolvePath(`nested\file.txt`)); got != want {
		t.Fatalf("EditTool.resolvePath() = %q, want %q", got, want)
	}
	if got := filepath.Clean(multiEditTool.ExportResolvePath(`nested\file.txt`)); got != want {
...
func TestWriteToolTreatsProvidedZeroMtimeAsConflictCheckInput(t *testing.T) {
	...
	tool := builtin.NewWriteToolWithOptions(root, root, nil)
...
func TestReadToolLimitZeroMeansNoLimit(t *testing.T) {
	...
	tool := builtin.NewReadToolWithOptions(root, root, nil)
	resp := tool.Run(map[string]any{
		"path":  "big.txt",
		"limit": 0,
	})
